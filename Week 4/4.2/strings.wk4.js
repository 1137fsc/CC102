...
function addNames () {
    ...
    if (/[^a-zA-Z\s]/.test(firstName.value + lastName.value)) {
        nameOver20.textContent = "Names only have letters!";
    } else if (firstName.value.trim() === "" || lastName.value.trim === "" ) {
        nameOver20.textContent = "Please input a first and last name";
    } else if (totalCount < 20 ) {  //if there are less than 20 characters in the total count
        nameOver20.innerHTML = `"your name is ${totalCount-1} characters" <br> That's less than 20 Characters. <br> The program continues...`; //send the output as the message
...
function checkZip() { //basically the same as above, just with different conditino checks
    ...
    const zipNumbersOnly = /^[0-9]+$/.test(zipCode.value);
...
window.onload = function() {
        // audio element by id
        const backgroundSound = document.getElementById("backgroundSpeech");
        ...
        playButton.addEventListener("click", function() {
            backgroundSound.play(); // self explanitory
            });
        // event listener to the pause button
        pauseButton.addEventListener("click", function() {
            backgroundSound.pause(); // Pause the sound
        });
};
// a bit of googling to get this... but basically take the curtime/duration and multiply to get it up to a percentage
//then use that percentage as the value for the progress bar. This is basically the same I do on HMI's for a meter or gauge display
//https://developer.mozilla.org/en-US/docs/Web/HTML/Element/progress
//https://developer.mozilla.org/en-US/docs/Web/API/HTMLMediaElement/currentTime
//https://developer.mozilla.org/en-US/docs/Web/API/HTMLMediaElement/duration
const audioProgress = document.getElementById("audioProgress");
backgroundSpeech.addEventListener("timeupdate", function() {
    audioProgress.value = (backgroundSpeech.currentTime / backgroundSpeech.duration) * 100;
});
